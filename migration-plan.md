...
<migration-log>
  ...
  <pr id="18" branch="staging (direct commits — Railway watch branch; see deviations)"
      base="staging" covers="The PR16/17 worker-tail, shipped to the working tree + locally verified: (a) worker globals.css now compiled by the customer's Tailwind v4 (esbuild→@tailwindcss/postcss, cwd-anchored via `base`); (b) non-fatal worker bundle failures (providers + globals) now write build_jobs.error instead of console.warn-and-swallow; (c) dashboard repo/branch link UUID bug fixed (de-demo'd dashboard branch-row + real workspace on repo-row). Ran the twice-deferred STEP-0a audit (clean). Redirect (P0) + discovery remain owner-gated → PR19. Autonomous code session."
      verified="tsc clean (web+api). (a) byte-proven LOCALLY (30,630 B with utilities, cwd-INDEPENDENT after the `base` fix) — LIVE NOT exercised (gated on reconnect→redirect). (b)+(c) tsc-clean + traced vs the instance route; LIVE NOT exercised. STEP-0a audit: no regressions, no new vulns, DO-NOT-REINTRODUCE list confirmed intact. Commits + push PENDING owner OK." date="2026-05-22">
    ...
    <deviations>
      - Direct staging commits + ask-before-push (PR14–17 precedent). Under this single migration-log row: commit 1 = worker pipeline (bundle.ts + lease.ts + worker.ts — fixes a+b combined; they share worker.ts and must land together to stay tsc-clean), commit 2 = dashboard link (branch-row + repo-row + dashboard-page, fix c), commit 3 = docs (migration-plan.md). PUSH PENDING owner OK at handback (NOT yet pushed/deployed).
    ...
    <known-risks>
      - **Redirect (P0, UNSOLVED, owner-gated):** localhost:8080 on BOTH install-Save and incognito login. All config hypotheses exhausted (source, config.toml, hosted Supabase Auth — REFUTED live in PR17, GitHub App fields). Next = the owner's DevTools Network "Preserve log" capture of each redirect chain → read the `Location` header per hop → identify the emitting server BEFORE any more theorising. Steps handed to the owner this session. Gates reconnect→rebuild→dropping the (a) manual-CSS crutch, AND login.
    ...
    <next>
      PR19 (owner-gated, design WITH owner): (1) the localhost:8080 redirect — owner runs the DevTools Network "Preserve log" capture (steps provided); read each `Location` header to find the emitting hop; then GitHub App slug/setup-url + duplicate-app check. Gates reconnect→rebuild→(a) goes live + the manual-CSS crutch removal. (2) component discovery (owner-flagged "unacceptable") + meaningful default content for contentless primitives — ask-user-question kickoff (heuristic vs config vs interactive picker). (3) verify the worker-tail LIVE once reconnect+rebuild is possible (confirm (a) self-produces globals.css, drop the crutch; confirm (b) surfaces any oxide-binary miss; click-test the (c) dashboard link). (4) demo.ts retirement (search-modal chips + recent-repos). Handover: plans/pr19-…md. Carry the OWNER MANDATE forward verbatim.
    </next>
  </pr>
</migration-log>
