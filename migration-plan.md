...
<migration-log>
  ...
  <pr id="18" branch="staging (direct commits — Railway watch branch; see deviations)"
    ...
    <verification gate="PR18 (worker-tail + audit)" result="CODE COMPLETE + tsc-clean (web+api); (a) byte-proven locally + cwd-independent; STEP-0a audit clean. LIVE NOT exercised for a/b/c (gated on reconnect→redirect). Commits/push pending owner OK.">
    ...
    <known-risks>
      - **Redirect fix (d) LIVE-UNVERIFIED:** root-caused by the owner's capture + fixed in code (publicOrigin from x-forwarded-host). Railway sets/overwrites x-forwarded-host (doc-confirmed) so confidence is high, but NOT proven live. GATE = the owner incognito-login smoke test post-deploy: lands on dashboard = FIXED; lands on localhost:8080 again = x-forwarded-host wasn't the header → add a one-shot debug-log of candidate request headers (or pin a public-URL env var). ROLLBACK if login breaks: `git revert <commit-4-sha> && git push origin staging` (60-sec undo). NOTE for custom-domain day: if a CDN is ever placed in front of Railway, Railway overwrites x-forwarded-host with the Railway host (not the CDN host) — revisit then.
      - **(a) LIVE-unverified + Railway-linux oxide:** the worker Tailwind compile is proven locally but never ran in the deployed worker; @tailwindcss/oxide (and lightningcss if `optimize` is later added) prebuilt native binaries must resolve on Railway-linux under --frozen-lockfile --ignore-scripts. First real rebuild confirms; a miss now surfaces on build_jobs.error (task b) instead of silently.
      ...
      - Carry-forward: dead install 134325166 (no rebuild until reconnect); active=FALSE recurrence on uninstall; app.ts WEB_ORIGIN CORS lockdown before go-live (0a security note).
    </known-risks>
