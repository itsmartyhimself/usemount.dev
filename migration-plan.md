...
<migration-log>
  ...
  <pr id="17" branch="staging (direct commits — Railway watch branch; see deviations)"
    ...
    <verification gate="PR17 (render)" result="PARTIAL — render fix verified LOCALLY; LIVE render NOT yet exercised">
      LOCAL GREEN: build:preview-runtime regenerates 4 files whose bytes carry the named exports (createRoot/hydrateRoot, jsx/jsxs/Fragment, react's 44 incl. createElement/useState); node ESM import resolves them to functions. Live console confirms the PR16 CORS error is gone. NOT YET: owner reload post-deploy to confirm the button VISIBLY renders and STAYS rendered (the STEP-0 gate). Pending push + Railway deploy. NOT done: worker Tailwind (task #3), error-surfacing (#4), dashboard-link (#5), the localhost:8080 redirect (Network capture next), discovery.
    </verification>
