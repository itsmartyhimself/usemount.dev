...
<migration-log>
  ...
  <pr id="17" branch="staging (direct commits — Railway watch branch; see deviations)"
      base="staging" covers="STEP-0 render: ROOT-CAUSED + FIXED the post-CORS blank render (preview-runtime modules had no named exports). Refuted the Supabase Site-URL redirect hypothesis by primary-source read. LIVE-DEBUG session with the owner. PR16-tail (worker Tailwind, error-surfacing, dashboard-link) + redirect Network-capture + discovery STILL in progress."
      verified="Render fix verified LOCALLY ONLY (regenerated bytes carry named exports; node ESM import resolves createRoot/jsx/createElement to functions) + the prior react.mjs CORS error is GONE from the owner's live console. LIVE visual render PENDING owner reload post-deploy." date="2026-05-22">
    ...
    <verification gate="PR17 (render)" result="PARTIAL — 3 render layers fixed; layers 1+2 CONFIRMED LIVE, layer 3 verified LOCALLY only">
      LAYER 1 (CORS): shipped PR16, CONFIRMED LIVE (console error gone). LAYER 2 (named exports): shipped 7237d55, CONFIRMED LIVE (createRoot SyntaxError gone). ERROR-SURFACING: shipped 7054926, CONFIRMED LIVE (the canvas now shows real iframe errors — that is how layer 3 was found). LAYER 3 (single React + forwardRef picker): verified LOCALLY ONLY — tsc clean (web+api); node loads the re-export chain; createRoot/createElement/useContext/jsx/createPortal resolve to functions; `R.useState === RDC.useState` proves a single shared React. NOT YET LIVE: owner reload to confirm the hook-using components (popover/animate-ui) VISIBLY render and STAY rendered — those are the discriminator (the hookless button was never the test). Pending push + Railway deploy. STILL NOT done: worker Tailwind (#3), worker-side error→build_jobs.error (#4 remainder), dashboard-link (#5), the localhost:8080 redirect (Network capture), discovery.
    </verification>

    <known-risks>
      - **Redirect (P0, UNSOLVED, one hypothesis fewer):** localhost:8080 on both install-Save and incognito-login; Supabase NOW refuted. Next = DevTools Network "Preserve log" capture of the redirect chain (which hop emits it), then GitHub App slug/setup-url + duplicate-app check.
      - **Render LIVE-unverified:** local-only so far; could surface a third downstream error on reload — if so, error-surfacing (#4) becomes the immediate next ship so the canvas reports it instead of another blind hunt.
      - Carry-forward (PR16): manual-CSS crutch; dead install 134325166 (no rebuild until reconnect, which needs the redirect); active=FALSE recurrence on uninstall; discovery too crude.
    </known-risks>

    <next>
      (1) Owner reloads → CONFIRM the button renders live (record result here). (2) During the deploy, owner runs the install-Save Network capture for the redirect. (3) Then worker Tailwind (#3) + error-surfacing (#4) + dashboard-link (#5), and discovery WITH the owner. Handover: plans/pr18-…md. Carry the OWNER MANDATE forward verbatim.
    </next>
  </pr>
</migration-log>
