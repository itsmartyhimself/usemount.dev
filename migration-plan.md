...
<migration-log>
  ...
  <pr id="20" branch="staging (direct commits — Railway watch branch; see deviations)"
      ...
      verified="LIVE-CONFIRMED. Leads (overflow + picker redesign) confirmed with the owner the prior session, incl. live jitter/spacing iteration. Demo retirement confirmed THIS (recovery) session via owner screenshots: the dashboard repo card + list row render real data with the 'Personal' workspace chip, and the search modal's 'Recently opened' row renders with its chip — proving the useWorkspaces() fetch → find(workspaceId) → WorkspaceChip chain. tsc clean (shared+api+web); iframe harness 41/41. NOT exercised live this session: the reconnect-deactivate path (tsc-clean, committed; needs a real reconnect) and the prod-CORS warn (log-line only — WEB_ORIGIN still UNSET on Railway, so cors() is still permissive)." date="2026-05-26">
    ...
    <verification gate="PR20 (overflow + picker redesign + reconnect fix + demo retirement)" result="LIVE-CONFIRMED for the leads (prior session, owner) + the demo retirement (this session, owner screenshots). tsc clean shared+api+web; iframe harness 41/41.">
