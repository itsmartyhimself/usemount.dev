...
<migration-log>
  ...
  <pr id="19" branch="staging (direct commits — Railway watch branch; see deviations)"
      ...
      verified="tsc CLEAN (shared+api+web) + REV's two authored files tsc-clean against REV's own types. LIVE NOT exercised — gated on (a) applying migration 0005 to live Supabase, (b) deploy (push pending owner OK), (c) owner clicks (pick components/plugin → rebuild → render) + (d) pushing the two REV files (pending owner OK). The failJob→instances.build_status='failed' consistency fix is code-verified, not live. Commits + pushes PENDING owner OK." date="2026-05-25">
    ...
    <verification gate="PR19 (picker + render composites + worker-tail surface)" result="CODE COMPLETE + tsc-CLEAN (shared+api+web); REV's two files tsc-clean against REV types. LIVE NOT exercised — pending migration-apply + deploy + owner clicks + REV push. NOT yet pushed/deployed.">
      tsc --noEmit clean across @usemount/shared (tsc -b), @usemount/api (tsc -b --force), @usemount/web (tsc --noEmit). REV's canvas.providers.tsx + plugin-container.preview.tsx tsc-clean against REV's own tsconfig (no errors referencing either file). The two REV files mirror proven-working REV source (app/page.tsx composition; the same TooltipProvider import pulse-tooltip.tsx already uses), so they bundle + render by construction. The picker's GitHub-tree counts use the SAME skip regex as the worker, so the shown counts equal what the build produces.
      NOT exercised LIVE (HONEST GAP, the whole point of the next agent / live loop): no migration applied, no deploy, no real picker save, no real rebuild against components/plugin, no PluginContainer-renders-its-composition confirmation, no PulseTooltip-with-provider confirmation, and the STEP-3 styling trio is untouched. The worker changes (multi-root scan, workDir-relative slugs, preview detection) are tsc-verified only — advisor agreed a local worker byte-check isn't warranted here (none of the PR17/18 preview-runtime fragility modes apply); live against REV is the decisive check.
    </verification>

    <known-risks>
      - **Deploy ordering (P0 for this PR):** apply migration 0005 to live Supabase FIRST, push SECOND. If the new web code deploys against a DB without preview_artifact_url, fetchInstanceRegistry's select errors → EMPTY_INSTANCE_REGISTRY → REV's sidebar goes EMPTY (regression). Migration is additive/nullable/idempotent so applying it before the old code keeps working.
      - **Worker behavior unverified-live:** multi-root + workDir-relative slugs + preview bundling are tsc-only. The first real REV rebuild is the test. The slug re-key is one-time (orphaned Storage objects; manifest IDs change).
      - **preview-example CSS dropped (v1):** an example that imports CSS the bare component doesn't would silently miss those styles. Fine for REV (Tailwind utilities via globals.css). PR20: upload previewBundle.cssBytes.
      - **STEP 3 styling trio still owed (PR20, gated on the first live rebuild):** confirm self-produced globals.<hash>.css → remove the manual globals.7f148…css crutch → reload + confirm styling holds. The (a) Railway-linux @tailwindcss/oxide native-binary risk under --frozen-lockfile --ignore-scripts is STILL unverified-live; task-b (now surfaced in the picker) is built precisely so a miss shows up.
      - Carry-forward: WEB_ORIGIN CORS lockdown before public launch (0a finding); demo.ts retirement (search-modal RepoRow chips + recent-repos); active=FALSE-on-uninstall recurrence (reconnect works). All PR20.
    </known-risks>

    <next>
      OWNER LIVE LOOP (one step at a time; owner clicks + reports, agent diagnoses): (0) apply migration 0005 to live Supabase (Supabase Dashboard → SQL Editor → paste 0005_picker_and_preview.sql → Run → 'Success'). (1) push staging on owner OK → Railway deploys → REV's EXISTING sidebar unchanged (preview_artifact_url=null fine). (2) owner opens REV → 'Choose folders' → ticks components/plugin (+ components/ui, components/ui/animate-ui for all surfaces) → Save → 'rebuilding' → reload → sidebar shows the plugin layer. EXPECTED at this point: most plugin components render; PluginContainer is an empty box; PulseTooltip is broken — this is the gap step 3 closes, TELL THE OWNER it'll look ugly so they don't think it broke. (3) push the two REV files on owner OK (show diff first) → webhook rebuild → StaleViewer toast → reload → PluginContainer renders its FULL real composition + PulseTooltip works. (4) STEP-3 styling trio (confirm self-produced globals.css → drop the manual crutch → reload + confirm styling holds). (5) UPDATE this entry's <verification> with the live outcome. Then PR20: STEP 4 (WEB_ORIGIN CORS, demo retirement), realtime refresh, nested folders, preview-CSS upload, non-PluginContainer examples. Handover: plans/pr20-…md. Carry the OWNER MANDATE forward verbatim.
    </next>
  </pr>
</migration-log>
