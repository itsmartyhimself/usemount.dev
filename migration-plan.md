...
<migration-log>
  ...
  <pr id="20" branch="staging (direct commits — Railway watch branch; see deviations)"
      ...
      verified="LIVE-CONFIRMED. Leads (overflow + picker redesign) confirmed with the owner the prior session, incl. live jitter/spacing iteration. Demo retirement confirmed THIS (recovery) session via owner screenshots: the dashboard repo card + list row render real data with the 'Personal' workspace chip, and the search modal's 'Recently opened' row renders with its chip — proving the useWorkspaces() fetch → find(workspaceId) → WorkspaceChip chain. Web tsc re-run clean THIS session (tsc --noEmit, exit 0); shared+api were touched only in Session A, which reported them tsc-clean + the iframe harness green at commit time — NOT re-run this (recovery) session (the harness round-trips the LIVE Supabase bucket/DB; not run unprompted against prod). Also NOT exercised live this session: the reconnect-deactivate path (tsc-clean, committed; needs a real reconnect) and the prod-CORS warn (log-line only — WEB_ORIGIN still UNSET on Railway, so cors() is still permissive)." date="2026-05-26">
    ...
    <verification gate="PR20 (overflow + picker redesign + reconnect fix + demo retirement)" result="LIVE-CONFIRMED for the leads (prior session, owner) + the demo retirement (this session, owner screenshots). Web tsc clean this session; shared+api Session-A-clean (unchanged in Session B); the iframe harness was NOT re-run this session.">
      Demo retirement (this session): owner screenshots show the dashboard 'Most accessed' card (itsmartyhimself/REV-plugin, main · 1 branches, synced 1d ago) + the All-repos list row (build failed · Personal chip) rendering REAL data, and the search modal 'Recently opened' row rendering with its Personal chip — the useWorkspaces()→find(workspaceId)→WorkspaceChip chain works (the {workspace ? <chip/> : null} gate would render nothing if resolution failed). tsc: web `tsc --noEmit -p tsconfig.json` exit 0.
